...
- Пользовательские триггеры:
  - пользователь может добавить свой вариант (сохраняется на сервере в аккаунте)
  - сразу доступен для выбора в приступе, без ожидания модерации
  - rate limit: не более `10` новых вариантов в день и `2` в минуту на пользователя
  - лимит: не более `50` своих вариантов на категорию (настраивается через `CUSTOM_OPTIONS_MAX_PER_CATEGORY`)
- Админ-модерация пользовательских триггеров (`/admin/triggers`) — для общего словаря, не блокирует использование
- Vue SPA маршруты:
  - `/login`
  - `/calendar`
  - `/attacks/new`
